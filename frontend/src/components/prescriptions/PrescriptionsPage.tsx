  ...
  useEffect(() => {
    const loadPrescriptions = async () => {
      ...
      if (!isDemoMode()) {
        ...
        if (token) {
          try {
            const response = await fetch(API_ENDPOINTS.PRESCRIPTIONS.PATIENT, {
              method: HTTP_METHODS.GET,
              headers: getAuthHeaders(token),
            });
            if (response.ok) {
              const data = await response.json();
              const mapped: Prescription[] = (data.prescriptions || []).map((p: any) => ({
                id: p.id,
                patientName: p.patientName || '',
                doctorName: p.doctorName || '',
                doctorSpecialization: p.doctorSpecialization || 'General Physician',
                diagnosis: p.diagnosis || '',
                medicines: (p.medicines || []).map((m: any) => ({
                  name: m.name || '',
                  dosage: m.dosage || '',
                  frequency: typeof m.frequency === 'object' ? formatFrequencyUtil(m.frequency) : (m.frequency || ''),
                  duration: m.duration || '',
                })),
                prescribedDate: p.prescribedDate || p.prescribed_date || '',
            ...
            console.error('Failed to fetch prescriptions from API, falling back to localStorage', err);
          }
        }
      }
