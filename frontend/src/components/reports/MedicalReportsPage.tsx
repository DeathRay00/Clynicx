...
import { analyzeMedicalReport, extractHealthTimelineData, clearRateLimitCooldown, type AnalysisResult } from '../../utils/reportAnalysisService';
...
import { getToken } from '../../utils/supabase/client';
...
// ── Backend API helpers ───────────────────────────────────────────────────────
const apiReports = {
  async fetchAll(): Promise<MedicalReport[]> {
    const token = getToken();
    if (!token) return [];
    const res = await fetch(API_ENDPOINTS.REPORTS.BASE, {
    ...
    return (data.reports ?? []).map((r: any) => ({
      ...
      fileName: r.fileUrl || 'report.pdf',
      fileSize: '-',
      reportType: (r.reportType as MedicalReport['reportType']) || 'other',
      dateUploaded: r.uploadDate?.split('T')[0] || r.createdAt?.split('T')[0] || new Date().toISOString().split('T')[0],
      reportDate: r.uploadDate?.split('T')[0] || new Date().toISOString().split('T')[0],
      status: r.aiAnalysis ? 'analyzed' : 'uploaded' as MedicalReport['status'],
      labName: r.labName,
      cost: r.cost,
  ...
  async create(report: Partial<MedicalReport> & { aiAnalysis?: AnalysisResult }): Promise<void> {
    const token = getToken();
    if (!token) return;
    await fetch(API_ENDPOINTS.REPORTS.BASE, {
      method: 'POST',
      headers: getAuthHeaders(token),
      body: JSON.stringify({
  ...
  async updateAnalysis(id: string, aiAnalysis: AnalysisResult): Promise<void> {
    const token = getToken();
    if (!token) return;
    await fetch(API_ENDPOINTS.REPORTS.BY_ID(id), {
      method: 'PUT',
      headers: getAuthHeaders(token),
...
export const MedicalReportsPage: React.FC = () => {
  ...
  const [retryTargetReport, setRetryTargetReport] = useState<MedicalReport | null>(null);

  // Load reports — from backend API when authenticated, localStorage in demo mode
  useEffect(() => {
    const loadReports = async () => {
      if (isDemoMode() || !getToken()) {
        ...
        // Real authenticated user — fetch from PostgreSQL backend
        try {
          const backendReports = await apiReports.fetchAll();
          setReports(backendReports);
        } catch (err) {
          console.error('Failed to load reports from backend, falling back to localStorage:', err);
          // Fallback
          let loadedReports: StoredReport[] = [];
          if (user?.role === 'patient' && user?.id) {
            loadedReports = getReportsForPatient(user.id);
    ...
    // Listen for same-tab storage events (demo mode)
    const handleReportUpdate = () => { if (isDemoMode() || !getToken()) loadReports(); };
    window.addEventListener('medicalReportUpdated', handleReportUpdate);
    return () => window.removeEventListener('medicalReportUpdated', handleReportUpdate);
  }, [user]);
  ...
  const handleFileUpload = async (file: File) => {
    ...
    toast.success('Report uploaded successfully!', {
      description: '🤖 AI is now analyzing your medical report...'
    });
    ...
    try {
      ...
      // Persist to backend (real users) or localStorage (demo)
      if (!isDemoMode() && getToken()) {
        try {
          ...
          // Reload from backend so the real UUID is used
          const backendReports = await apiReports.fetchAll();
          setReports(backendReports);
          // Remove the optimistic placeholder
          setReports(prev => {
            const withoutPlaceholder = prev.filter(r => r.id !== reportId);
            return [...backendReports.filter(r => !withoutPlaceholder.some(e => e.id === r.id)), ...withoutPlaceholder];
          });
        } catch (apiErr) {
          console.error('Backend save failed, keeping in localStorage:', apiErr);
          updateReport(reportId, { status: 'analyzed', reportType: detectedReportType, aiAnalysis: analysis });
        }
      } else {
      ...
      setReports(prev => prev.map(r => 
        r.id === reportId 
          ? { ...r, status: 'analyzed' as const, reportType: detectedReportType, aiAnalysis: analysis }
          : r
      ));
      
      // Show appropriate success message based on analysis type
      if (analysis.isMockAnalysis) {
        toast.warning('⚠️ AI Analysis Unavailable', {
          description: analysis.errorMessage || 'Using sample analysis based on report type.',
          duration: 5000
        });
      } else {
      ...
      updateReport(reportId, { status: 'uploaded' });
      setReports(prev => prev.map(r => 
        r.id === reportId ? { ...r, status: 'uploaded' as const } : r
      ));
      toast.error('AI analysis failed. You can retry later.');
    }
  };

  // Retry AI analysis for a report that got mock/sample analysis
  const handleRetryAIAnalysis = (report: MedicalReport) => {
    clearRateLimitCooldown();
    setRetryTargetReport(report);
  ...
  const handleRetryFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !retryTargetReport) return;
    // Reset input so same file can be re-selected next time
    e.target.value = '';

    if (file.type !== 'application/pdf') {
      toast.error('Please select the original PDF file');
      return;
    }
    ...
    setReports(prev => prev.map(r =>
      r.id === retryTargetReport.id ? { ...r, status: 'analyzing' as const } : r
    ));
    ...
    try {
      ...
      if (!isDemoMode() && getToken()) {
        try { await apiReports.updateAnalysis(retryTargetReport.id, analysis); } catch {}
      } else {
        updateReport(retryTargetReport.id, { status: 'analyzed', reportType: detectedReportType, aiAnalysis: analysis });
      }

      setReports(prev => prev.map(r =>
        r.id === retryTargetReport.id
          ? { ...r, status: 'analyzed' as const, reportType: detectedReportType, aiAnalysis: analysis }
          : r
      ));

      if (analysis.isMockAnalysis) {
        toast.warning('⚠️ AI still unavailable', {
          description: analysis.errorMessage || 'Still showing sample analysis. Try again later.',
          duration: 5000
        });
      } else {
        const reportDate = retryTargetReport.reportDate;
        const timelineData = extractHealthTimelineData(analysis, retryTargetReport.id, reportDate);
        if (timelineData.length > 0 && user?.id) {
          addMultipleTimelineEntries(user.id, timelineData);
        }
        toast.success('🎉 AI analysis complete!', {
          description: 'Your medical report has been successfully analyzed by AI.'
        });
      }
    } catch (error) {
      console.error('Retry analysis failed:', error);
      updateReport(retryTargetReport.id, { status: 'analyzed' });
      setReports(prev => prev.map(r =>
        r.id === retryTargetReport.id ? { ...r, status: 'analyzed' as const } : r
      ));
      toast.error('AI analysis failed again. Please try later.');
    } finally {
      setRetryTargetReport(null);
    }
  };
  ...
  const handleDeleteReport = async (reportId: string, fileName: string) => {
    ...
    try {
      if (!isDemoMode() && getToken()) {
        await apiReports.remove(reportId);
      } else {
        const ok = deleteReport(reportId);
        if (!ok) throw new Error('localStorage delete failed');
      }
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Hidden file input for retry AI analysis */}
      <input
        ref={retryFileInputRef}
        type="file"
        accept=".pdf"
        aria-label="Select PDF for AI retry analysis"
        className="hidden"
                  ...
                  {report.aiAnalysis && (
                    <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {report.aiAnalysis.isMockAnalysis && (
                      ...
                      {report.aiAnalysis.isMockAnalysis && (
                        <div className="mt-2 flex items-center gap-2 text-xs text-yellow-700">
                          ...
                          <span>AI could not analyze this report (rate limit). Re-upload the PDF to retry.</span>
                          <Button
                            variant="outline"
                            size="sm"
                            className="ml-auto text-xs h-7 px-2 border-yellow-300 text-yellow-700 hover:bg-yellow-50"
                            onClick={() => handleRetryAIAnalysis(report)}
                          >
              ...
              {selectedReport.aiAnalysis && (
                <div className="space-y-6">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {selectedReport.aiAnalysis.isMockAnalysis && (
                  ...
                  {/* Mock Analysis Warning */}
                  {selectedReport.aiAnalysis.isMockAnalysis && selectedReport.aiAnalysis.errorMessage && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                      <p className="text-sm text-yellow-800">
                        ℹ️ {selectedReport.aiAnalysis.errorMessage}
                      </p>
                    </div>
                  )}
